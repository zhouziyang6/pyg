app.controller("goodsController", function ($scope, $controller, $location, goodsService, uploadService,
    ...
    //商品的上下架状态
    $scope.goodsStatus2 = ["未上架", "上架中"];
    //修改商品的上架状态
    $scope.itemUpshelf = function () {
        if ($scope.selectedIds.length < 1) {
            alert("请先选择商品");
            return;
        }
        if (confirm("确定要上架选中的商品吗？")) {

            goodsService.itemUpshelf($scope.selectedIds).success(function (response) {
                if (response.success) {
                    //刷新列表并清空选中的那些商品
                    $scope.reloadList();
                    $scope.selectedIds = [];
